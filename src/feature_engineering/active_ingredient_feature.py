...
class ActiveIngredientsFeature(BaseFeature):
    ...
    def fit(self, X, y=None):
        ...
        self.svd = TruncatedSVD(n_components=config.n_components_active_ingrendients_svd)
        self.svd.fit(X)
        return self
    ...
    def transform(self, X):
        ...
        return pd.DataFrame(
            ...
            columns=[f"active_ingredient_feature_{i}" for i in range(1, config.n_components_active_ingrendients_svd+ 1)]
        )

    def get_feature_names(self):
        return [f"active_ingredient_feature_{i}" for i in range(1, config.n_components_active_ingrendients_svd+ 1)]
